...
一个隐私优先的端侧 AI 语言暴力识别与主动干预开源系统。

本项目可在 Raspberry Pi 5、Mac Mini 或普通 x86_64 PC 上运行，通过本地语音识别、声学风险分析、规则引擎、LLM 语义分析和 Web 控制台，对家庭、伴侣沟通、教育陪伴和儿童保护等场景中的高风险语言互动进行识别、提醒、记录和人工复盘。

> 本项目不是医疗诊断、法律证据或紧急救援替代系统。所有检测结果仅作为辅助信号，必须保留人工判断空间。

语言: [中文](README.md) | [English](README_EN.md)

快速入口: [界面截图](#使用界面截图与演示效果) · [快速安装](#快速安装) · [树莓派部署](#树莓派-5-部署) · [专利声明](PATENT_NOTICE.md) · [路线图](ROADMAP.md) · [许可证](#许可证)

## 专利申请状态与开源边界

本项目相关技术已提交中国发明专利申请，申请号 `202610669466.3`，申请日 `2026-05-15`，并已收到发文日为 `2026-06-25` 的发明专利申请初步审查合格通知书。该状态表示申请仍在审查流程中，并不表示已经授权。

本仓库公开的是开源样机源码、配置示例、匿名演示材料和部署说明，不公开专利申请全文、权利要求书、说明书附图、审查往来文件或未公开实施资料。Apache License 2.0 的专利许可仅按许可证第 3 节适用于本仓库明确发布的 Work，不额外授权未公开实现、硬件设计、真实数据、未来改进方案或另行申请的专利。

发布或贡献新内容前，请先按 [Open-Source Patent Review Checklist](OPEN_SOURCE_PATENT_REVIEW.md) 检查是否包含未申请的新发明点、真实数据、密钥或专利申请材料。详细边界见 [PATENT_NOTICE.md](PATENT_NOTICE.md)。

## 30 秒了解

- **它是什么**：一个端侧运行的语言暴力识别、温和提醒和人工复盘样机。
- **能做什么**：识别高风险表达、声学异常和趋势变化，并通过 TTS 播放非攻击性提醒。
- **在哪里运行**：Raspberry Pi 5、Mac Mini、通用 x86_64 PC，也给出了其他 ARM 设备迁移建议。
- **如何保护隐私**：默认不保存原始录音；事件日志、声纹模板、人员画像、筛查结果和安全处置记录均本地保存。
- **当前阶段**：`v0.1.0-alpha` 样机版，已完成 WebUI、部署脚本、声纹/媒体过滤/趋势报告等核心链路，仍需真实场景误报评估。

## 核心特点

- **本地优先**：默认不保存原始录音，支持本地 Whisper 转写。
- **主动干预**：检测到高风险表达后，可播放温和提醒话术。
- **双引擎检测**：规则引擎快速预检 + LLM 深度语义分析。
- **Web 控制台**：查看事件、趋势、人员画像、声纹管理、媒体声过滤和安全处置。
- **树莓派部署**：支持 Raspberry Pi 5 长期运行，并提供 systemd 服务脚本。
- **隐私保护**：日志、声纹模板、心理筛查和安全处置记录均以本地私有文件保存。

## 项目概述

本项目实现了一套可在 Raspberry Pi 5 上持续运行的语言暴力检测与主动干预样机，提供本地检测服务和受保护的 Web 管理控制台。项目定位是“隐私优先的家庭安全 / 语言暴力识别 / 端侧 AI 样机”，不主张用于家庭监控、人员控制、法律取证或心理诊断。
...
本项目代码以 Apache License 2.0 开源。请保留 `LICENSE`、`NOTICE`、`LEGAL_NOTICE.md` 与 `PATENT_NOTICE.md` 中的 `satantqr0` 版权署名、项目来源、专利提示和免责声明。
...
- Apache License 2.0 包含其第 3 节规定的专利授权和专利诉讼终止条款；仓库声明不额外授权硬件设计、未公开实现、部署数据、录音、声纹模板、人员画像、心理筛查记录、安全处置记录、未来改进方案或另行申请的专利。
- 第三方依赖、模型权重、云服务、语音服务和系统软件仍受各自许可证或服务条款约束，详见 `THIRD_PARTY_NOTICES.md`。

另见 `LEGAL_NOTICE.md` 与 `PATENT_NOTICE.md`：本项目不是医疗诊断、法律证据或紧急救援替代系统；涉及人员识别、画像和筛查时必须取得合法依据与必要同意；涉及专利申请和新增技术方案时应先完成公开风险评估。
