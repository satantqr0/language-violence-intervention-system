...
本目录是专利申请受理后整理出的开源准备版。公开前请先对照专利申请文件确认：仓库中公开的技术内容已经被申请文件支持；申请日之后新增、且未写入申请文件的改进点已决定公开或已另行提交补充/改进申请。

请不要提交真实 `.env`、API key、SSH key、部署密码、运行日志、录音、声纹模板、人员画像、心理筛查或安全处置记录。公开前可按 `OPEN_SOURCE_CHECKLIST.md` 再检查一遍。
